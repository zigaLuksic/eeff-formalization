...
| VeqLeft A B Γ v v' :
    veq A Γ v v' ->
    veq' (TySum A B) Γ (Left v) (Left v')
| VeqRight A B Γ v v' :
    veq B Γ v v' ->
    veq' (TySum A B) Γ (Right v) (Right v')
| VeqNil A Γ :
    veq' (TyList A) Γ Nil Nil
...
| βMatchPair v1 v2 c C Γ: 
    ceq' C Γ
      ...
      (c_subs2_out c v1 v2)
| βMatchLeft v c1 c2 C Γ:
    ceq' C Γ
      (SumMatch (Left v) c1 c2)
      (c_subs_out c1 v)
| βMatchRight v c1 c2 C Γ:
    ceq' C Γ 
...
| βDoOp op v c1 c2 C Γ:
    ceq' C Γ
      (Do (Op op v c1) c2)
      (Op op v (Do c1 (c_shift c2 1 1)))
| βHandleRet c_r h v C Γ:
    ceq' C Γ
      (Handle (Handler c_r h) (Ret v))
      (c_subs_out c_r v)
| βHandleOp c_r h op v c_k c_op C Γ:
