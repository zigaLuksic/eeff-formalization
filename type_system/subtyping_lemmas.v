...
eapply Respects; auto. apply RespectAlways.
}
Qed.
...
+ eapply sig_subtype_get_Some in H. 2: exact sty.
  ...
  eapply WfTOp; eauto.
  - eapply ctx_subtype_vtype; eauto. eapply TypeV.
    inv H0. auto. apply get_op_type_wf in gets'; auto. destruct gets'. auto.
    eapply TypeVSubsume; eauto.
  - apply IHorig; auto.
    * apply STyCtxU; auto.
    * apply WfCtxU. all: auto.
    apply get_op_type_wf in gets'; auto. destruct gets'. auto.
Qed.
...
+ rewrite same in H2. apply subtype_shape_sum in H2. 
  ...
  destruct H0.
  * left. destruct H0 as [v1[same tys]].
    exists v1. aconstructor.
    apply TypeV; auto. eapply TypeVSubsume; eauto.
  * right. destruct H0 as [v2[same tys]].
    exists v2. aconstructor.
    apply TypeV; auto. eapply TypeVSubsume; eauto.
Qed.


Fixpoint shape_left Γ v A v' (orig : has_vtype Γ v A) {struct orig} :
  v = Left v' ->
  exists A' B',
...
Fixpoint shape_right Γ v A v' (orig : has_vtype Γ v A) {struct orig} :
  v = Right v' ->
  exists A' B',
...
+ rewrite same in H2. apply subtype_shape_fun in H2. 
  ...
  destruct H1. auto. right. destruct H1 as [c'[same ctys]]. 
  exists c'.
  aconstructor. apply TypeC; inv H0; auto.
  * apply WfCtxU; auto.
...
+ subst. apply (shape_handler_full _ _ c_r h) in H1; eauto.
  ...
  - apply Respects; auto. inv hty. auto. inv hty. auto.
    inv H0. inv H6. eapply wf_eqs_sig_subtype; eauto. inv H3. inv H4.
...
eapply (shape_absurd_full  _ _ _ v) in H1; auto.
Qed.
