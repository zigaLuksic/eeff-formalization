...
+ apply shape_summatch in orig.
  ...
  destruct vty as [A''[B''[same tys]]]. inv same. auto. 
+ apply shape_summatch in orig.
...
+ apply shape_app in orig. destruct orig as [A'[ty1 ty2]].
  eapply c_subs_out_typesafe; eauto.
+ eapply shape_letrec_full in orig. 2: reflexivity.
  destruct orig as [A[C[c1ty c2ty]]].
  ...
  - eapply TypeLetRec; auto.
    ...
    inv c2ty. inv H. inv H5. all: auto.
...
+ destruct C' as (Ac, Σ, E) eqn:e. apply shape_do in orig.
  ...
  destruct c1ty as [Aop[Bop[gets[vty cty]]]]. apply TypeC.
  - inv vty. auto.
  - inv H. auto.
  - eapply TypeOp; eauto. 
    * eapply TypeC. inv cty. auto. inv H. auto.
      eapply TypeDo; eauto.
      assert (CtxU (CtxU Γ Bop) A' = ctx_insert (CtxU Γ A') 1 Bop) as same.
...
+ eapply shape_handle in orig. rename C' into D. destruct orig as [C[hty]].
  ...
  destruct hty' as [Σ'[D'[retty[hcty[r[sty csty]]]]]]. eapply shape_ret in H.
...
+ eapply shape_handle in orig. rename C' into D. 
  ...
  destruct hty' as [Σ'[D'[retty[hcty[r[stysig styd]]]]]].
  eapply shape_op_full in H0 as opt; eauto.
  destruct opt as[Aop[Bop[gets[vty cty]]]].
  ...
  - eapply sig_subtype_get_Some in gets; eauto.
    ...
    * apply STyCtxU. apply STyCtxU.
      ...
      apply STyFun; auto. eauto.
      apply csubtype_refl. auto. apply vsubtype_refl. auto. 
      apply WfTyFun; auto. apply get_op_type_wf in gets'. destruct gets'. auto.
      inv hcty. auto.
  - apply v_shift_typesafe; auto.
    ...
    eapply TypeVSubsume. instantiate (1:=(TyHandler (CTy Ac Σ' E) D')).
    * inv H0. inv H3.
    ...
    * inv H0. inv H3.
      ...
      apply vsubtype_refl; auto. eapply eqs_subtype_refl; eauto.
...
+ right. right. clear IHc2. destruct C as (A, Σ, E). 
  ...
  - destruct h2. destruct H0 as [v[c same]]. rewrite same in *.
...
+ right. right. eapply shape_handle in orig. rename C into D.
  ...
  - destruct H1 as [op[v[c']]]. subst.
    destruct H0 as [hcsty[r[sigsty styd]]].
    eapply shape_op_full in H; eauto.
    destruct H as [A_op[B_op[gets[vty cty]]]].
