...
| Step_smatch_r v xl cl xr cr:
    small_step 
      ...
      (csub_out cr v)
| Step_apply x c v:
    small_step (App (Fun x c) v) (csub_out c v)
...
| Step_letrec f x f_ty c1 c2:
    small_step
      ...
      (csub_out c2 (Fun x (LetRec f x f_ty c1 c1)))
| Step_dobind_ret x v c:
    small_step (DoBind x (Ret v) c) (csub_out c v)
...
| Step_handle_op x c_r h op_id v_arg y c_k x_op k_op c_op :
    ...
    small_step
      ...
      (csub_out 
        ( csub_out c_op (Fun y (Handle (Handler x c_r h) c_k)) )
        v_arg )
.