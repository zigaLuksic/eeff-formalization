...
Lemma v_no_var_shift (v:val) j cut:
  v_no_var v j -> (cut <= j) -> v_no_var (Sub.v_shift v 1 cut) (1+j)

with c_no_var_shift (c:comp) j cut:
  c_no_var c j -> (cut <= j) -> c_no_var (Sub.c_shift c 1 cut) (1+j)
  
with h_no_var_shift (h:hcases) j cut:
  h_no_var h j -> (cut <= j) -> h_no_var (Sub.h_shift h 1 cut) (1+j).
Proof.
{
